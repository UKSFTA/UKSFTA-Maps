...
[
    {
        ...
        // 1. UNIT DISCOVERY & PRIORITY (Every 120 frames / ~2s)
        if (UKSFTA_Accum_Counter % 120 == 0) then {
            ...
            // Cap high-fidelity updates to the 12 closest units
        ...
        // 2. STAGGERED UNIT PROCESSING
        // We process 3 units per frame to flatten the CPU load
        ...
        for "_i" from _startIndex to _endIndex do {
            ...
            if (isNil "_unit" || {isNull _unit}) continue;

            // --- 2a. MATHEMATICAL ACCUMULATION (Owner Only) ---
            if (local _unit) then {
                // Throttle calculation based on distance
                private _dist = _unit distance player;
                private _calcChance = if (_dist < 20) then { 1.0 } else { 0.2 };
            ...
            if (_uiName != "") then {
                ...
                if (!isNil "_display" && {!isNull _display}) then {
                    // Update fades only if unit is very close or looking at them
                    if (_unit distance player < 30) then {
                        ...
                        { _x ctrlCommit 0.5; } forEach [(_display displayCtrl 101), (_display displayCtrl 103), (_display displayCtrl 104)];
                    };
                };
            };
        };
    },
    0 // Run every frame, but logic is internally staggered
