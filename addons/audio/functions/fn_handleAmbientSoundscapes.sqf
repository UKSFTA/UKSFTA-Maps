    ...
    while {missionNamespace getVariable ["uksfta_environment_enabled", true]} do {
        ...
        switch (_biome) do {
            ...
            default { // TEMPERATE / WOODLAND
                // A3\Sounds_F\environment\ambient\forest\A3\Sounds_F\environment\ambient\forest\forest_wind_2.wss is missing in some vanilla versions, using A3\Sounds_F\environment\ambient\forest\forest_wind_2.wss instead
                _sound = ["A3\Sounds_F\environment\ambient\forest\A3\Sounds_F\environment\ambient\forest\forest_wind_2.wss", "A3\Sounds_F\environment\ambient\night\night_crickets_1.wss"] select (_isNight);
                _vol = 0.5;
            };
        };
